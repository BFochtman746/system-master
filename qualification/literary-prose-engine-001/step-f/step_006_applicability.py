...
def plan_applicability(features, context):
    ...
    This does not judge literary quality. It only decides which authorities have enough
    local reason/context to inspect a passage. Absence from the plan means NOT_APPLICABLE,
    not PASS.
    """
    owners = {}
    ...
    if features.get("character_state_material"):
        dims = ["ACTIVE_GOAL","MOTIVE_VALUE_PRESSURE","BELIEF_STATE","KNOWLEDGE_STATE",
        ...
        add("CHARACTER_STATE_ARC_v2", dims, "character state/choice/arc is materially present")

    if features.get("dialogue_material"):
        dims = ["DIALOGUE_FUNCTION","SPEAKER_OBJECTIVE","SPEECH_ACT","IMPLICATURE","SUBTEXT",
        ...
        add("DIALOGUE_PRAGMATICS_CHARACTER_v2", dims, "dialogue/pragmatic action is materially present")

    if features.get("originality_question"):
        dims = ["CONCEPT_DISTINCTIVENESS","SCENE_SOLUTION_DISTINCTIVENESS","EVENT_TURN_PREDICTABILITY",
        ...
        add("ORIGINALITY_DISTINCTIVENESS_v1", dims, "distinctiveness/predictability question is material")

    if features.get("theme_motif_material"):
        dims = ["THEME_HYPOTHESIS","THEMATIC_EVIDENCE","ACTION_EMBODIMENT","SUBTEXT_INFERENCE_CHAIN",
        ...
        add("THEME_SUBTEXT_MOTIF_v1", dims, "theme/subtext/motif evidence is materially present")

    if features.get("pacing_material"):
        dims = ["STORY_DISCOURSE_TIME","TEMPORAL_ORDER","SCENE_DURATION","SUMMARY_DURATION","PAUSE_DURATION",
        ...
        add("PACING_NARRATIVE_TIME_v2", dims, "narrative-time/pacing structure is materially in question")
    ...
    add("PACKET_012_PROSE_CRAFT", prose_dims, "specific prose-craft family is materially implicated")

...
set "OBJECTIVE=BOOK-EVAL-LEMONADE-001-BLIND-RESUME-001"
...
set "EVIDENCE=%RUNNER_TEMP%\book-eval-blind-resume-%GITHUB_RUN_ID%"
...
set "SUMMARY=%EVIDENCE%\blind-resume-summary.txt"
...
(
 ...
 echo preexisting_e5_expected=16
)>"%SUMMARY%"
...
if not "!E5_BEFORE!"=="16" (set "FAIL=e5_precondition_not_16" & goto :fail)
certutil -hashfile "%E4_FILE%" SHA256 >"%EVIDENCE%\e4-before-sha256.txt" 2>&1 || (set "FAIL=e4_before_hash_failed" & goto :fail)
for /f "tokens=*" %%H in ('findstr /r /i "^[0-9a-f][0-9a-f]*$" "%EVIDENCE%\e4-before-sha256.txt"') do set "E4_SHA_BEFORE=%%H"
if not defined E4_SHA_BEFORE (set "FAIL=e4_before_hash_parse_failed" & goto :fail)

echo e4_before_cases=!E4_BEFORE!>>"%SUMMARY%"
echo e5_before_cases=!E5_BEFORE!>>"%SUMMARY%"
echo e4_before_sha256=!E4_SHA_BEFORE!>>"%SUMMARY%"
...
set "E4_SHA_NOW="
for /f "tokens=*" %%H in ('findstr /r /i "^[0-9a-f][0-9a-f]*$" "%EVIDENCE%\e4-pass-!PASS!-sha256.txt"') do set "E4_SHA_NOW=%%H"
if /I not "!E4_SHA_NOW!"=="!E4_SHA_BEFORE!" (set "FAIL=e4_bytes_changed_during_resume" & goto :fail)
...
(
 ...
 echo qualification=PASS_BLIND_RESUME_E5_COMPLETE
)>>"%SUMMARY%"
type "%SUMMARY%"
exit /b 0

:incomplete
set "FAIL=e5_not_complete_after_12_resumable_passes"
goto :fail
...
if errorlevel 1 (set "FAIL=%LABEL%_decode" & exit /b 1)
certutil -hashfile "%TARGET_FILE%" SHA256 >"%EVIDENCE%\hash-%LABEL%.txt" 2>&1
if errorlevel 1 (set "FAIL=%LABEL%_hash_command" & exit /b 1)
findstr /i /c:"%EXPECTED_SHA%" "%EVIDENCE%\hash-%LABEL%.txt" >nul
if errorlevel 1 (set "FAIL=%LABEL%_hash_mismatch" & exit /b 1)
echo %LABEL%_sha256=%EXPECTED_SHA%>>"%SUMMARY%"
echo %LABEL%_part_count=!ACTUAL_COUNT!>>"%SUMMARY%"
exit /b 0

:fail
if not defined FAIL set "FAIL=unspecified"
echo failure=!FAIL!>>"%SUMMARY%"
echo qualification=FAIL_BLIND_RESUME>>"%SUMMARY%"
if exist "%STATUS_FILE%" copy /y "%STATUS_FILE%" "%EVIDENCE%\BLIND-RUN-STATUS.json" >nul
type "%SUMMARY%"
exit /b 1

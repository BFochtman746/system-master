# EXPERIENCE — Foundation Contract 001

**Capability** `C12` · **Owner** `SYSTEM_MASTER/CORE` · **Lane** `CORE`
...
Operations: `compose_view_model`, `present_progress`, `present_decision`, `record_ui_intent`, `restore_ui_state`.
...
Chat/system state changes, capability progress, permission/decision requests.
...
UI/view models, progress states, decision records, accessibility metadata.
...
**Canonical semantic writer:** `SYSTEM_MASTER/CORE`. CORE Experience service writes durable UX/session preference and decision state; transient rendering state is not durable authority.
...
Future evidence target (currently absent until implementation qualification): `qualification/foundation/experience-foundation-001.json`.
Required contents: `C12`, `EXPERIENCE`, owner `SYSTEM_MASTER/CORE`, current authority/crosswalk identifiers, exact subject Git blobs, route coverage, canonical-writer assertion, dependency/failure/idempotency test results, acceptance command, result `PASS|FAIL`, and immutable evidence/artifact references.

## 8. Acceptance target

Pre-code specification gate: `node .github/scripts/foundation-capability-contract-spec-check.js EXPERIENCE`.
Implementation acceptance gate: `node .github/scripts/foundation-capability-acceptance.js EXPERIENCE`.

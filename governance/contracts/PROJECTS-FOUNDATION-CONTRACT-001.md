# PROJECTS — Foundation Contract 001

**Capability** `C29` · **Owner** `SYSTEM_MASTER/CORE` · **Lane** `CORE`
...
Operations: `create_project`, `update_project`, `attach_artifact`, `set_milestone`, `record_progress`, `close_project`.
...
User/Chat or authorized capability request with project identity and mutation intent.
...
project state, work/milestone refs, artifact links, progress events.
...
**Canonical semantic writer:** `SYSTEM_MASTER/CORE`. CORE Projects service is sole semantic writer for canonical project/workspace graph state.
...
Future evidence target (currently absent until implementation qualification): `qualification/foundation/projects-foundation-001.json`.
Required contents: `C29`, `PROJECTS`, owner `SYSTEM_MASTER/CORE`, current authority/crosswalk identifiers, exact subject Git blobs, route coverage, canonical-writer assertion, dependency/failure/idempotency test results, acceptance command, result `PASS|FAIL`, and immutable evidence/artifact references.

## 8. Acceptance target

Pre-code specification gate: `node .github/scripts/foundation-capability-contract-spec-check.js PROJECTS`.
Implementation acceptance gate: `node .github/scripts/foundation-capability-acceptance.js PROJECTS`.

...
**Capability** `C04` · **Owner** `SYSTEM_MASTER/CORE` · **Lane** `CORE`
...
Operations: `submit_turn`, `stream_response`, `attach_artifact`, `record_next_step`, `resume_conversation`.
...
User turn, attachment/import event, approved system follow-up, or background-job completion.
...
assistant response stream, artifact refs, next-step/action proposals, status/progress events.
...
**Canonical semantic writer:** `SYSTEM_MASTER/CORE`. CORE Chat service writes conversation/session state and artifact links; owner systems write their own domain state; CORE artifact/evidence stores write shared physical records.
...
`qualification/foundation/chat-foundation-001.json`.
Required contents: `C04`, `CHAT`, owner `SYSTEM_MASTER/CORE`, current authority/crosswalk identifiers, exact subject Git blobs, route coverage, canonical-writer assertion, dependency/failure/idempotency test results, acceptance command, result `PASS|FAIL`, and immutable evidence/artifact references.

## 8. Acceptance target

Pre-code specification gate: `node .github/scripts/foundation-capability-contract-spec-check.js CHAT`.
Implementation acceptance gate: `node .github/scripts/foundation-capability-acceptance.js CHAT`.

...
- `.github/workflows/p15-a01-morning-receipt.yml`
  - daily read-only A-01 receipt after the New York shift boundary plus manual dispatch.
...
3. Daily `[self-hosted, Windows, X64]` scheduled receipt at `15 12 * * *` UTC, which is after 07:00 America/New_York in both EDT and EST.
...
- Scheduled workflow artifact retained for 90 days.
...
`0`, `1`, and `2` are intentionally distinct. **Nothing ran** and **I could not tell** must never collapse into the same result. Scheduled exit `1` or `2` deliberately surfaces as a red workflow after the artifact is preserved.
...
The optional output file/artifact is caller-owned reporting output, not canonical execution state. Rollback history is append-only and event-created in `governance/ROLLBACK-LOG-001.md` when the first real rollback occurs; P15 does not fabricate a rollback log before an event exists.
...
- daily schedule exists after the complete shift boundary and preserves a receipt artifact;
...
**PASS** requires all **16 tests** with zero failures/skips, hosted P15 manifest upload, trusted A-01 admission, exact-subject A-01 `PASS`, `promotion_authorized=false`, and independent Required Verification.
...
**Agent-owned:** diagnostic query optimization, section wording that preserves decision-first meaning, additional read-only fields, tests, schedule/report packaging, evidence manifests, and rollback documentation that does not create new authority.

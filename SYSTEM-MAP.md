...
`.github/scripts/document-truth-qualify.py` fails the suite when this document or `CURRENT-STATE.md` asserts a file the tree does not contain.

**Why it exists.** The most expensive defects in this recovery were not broken code — they were documents asserting capability the tree lacked, which the next session then planned against. The bootstrap inventory above was wrong in every particular and cost a full pass to re-derive. PR #239's body claimed both documents recorded the poller's honest scope; the diff contained only two Java files, so PR #240 existed for no purpose but to correct that. Both were mechanically detectable at the moment of the claim, and nothing detected them because no gate read the prose.

**What it checks.** Every backticked token that is unambiguously a repo file path — carries a code suffix, no glob, no ellipsis, not build output — must resolve in the tree. The narrowness is deliberate: `control-gateway/**` is a path filter, `SYSTEM_MASTER/CORE` is a subsystem, and a first version of this logic that treated those as file claims reported 46 violations where there was one.

**Absence stays sayable.** A line carrying an absence marker (`off main`, `sole custodian`, `abandoned`, `never landed`, ...) exempts its paths, because the "Known absent" and off-`main` disposition sections are the most valuable lines here. A gate that punished naming a missing file would pressure this document into silence, which is the same disease in the other direction.

**It proves itself on every run.** The first version of this audit skipped directories with `".git" in dirpath`, which also skips `.github/` — so every workflow and qualifier path reported missing and the gate-set comparison inspected zero files. A check passing because it looked at nothing is the same class of defect as the reporter that parsed only TAP and fail-closed to `pass=0 fail=-1`. Six negative self-proofs therefore run before every real check and a self-proof failure is a hard failure: the gate cannot report clean unless it has just demonstrated it can still catch a planted lie and still see into `.github/`. Mutation-proved in both directions — planting a false path fails the check; restoring the substring skip fails the self-proofs.
...
- **Run polling exists but is not wired into production.** Corrected 2026-09-17: an earlier revision said `ActionsRunPoller` was branch-only, compiled, and untested. It is now on `main` (`36d0037c`) with 47 passing checks and five mutation proofs. What remains absent is the **wiring**: `ClaimDriverMain` still constructs a bare `ActionsDispatch`, so for scheduled runs `DONE` still attests acceptance only. Two preconditions blocked wiring; **the first is now closed** — `claim-work.yml` sets `run-name: Claim Work [<claim id>]`, so the run title carries the claim id, and `ClaimWorkCorrelationTest` reads that YAML and proves the rendered title correlates through the real matcher, so the workflow and the Java cannot drift apart silently. Still open: the **wiring itself** — `ClaimDriverMain` is unchanged. Precondition 2 is now closed: `ActionsRuns` is the production `Runs` implementation (45 checks), so the query seam is no longer fakes-only. One correction worth recording, because it changed the design: an earlier note here said no workflow sets `run-name`. That was true of the *tree* but **not of the run history** — completed runs already exist titled `claim:durability-003-live-35035058758-1 dispatch:af66fb49-…`, i.e. carrying claim ids in the correlating shape. Correlation is by title, so an unbounded run query lets a months-old run satisfy a claim dispatched seconds ago. `ActionsRuns` therefore bounds every query with a `now - lookback` created-at floor, applied both as a `created=>=` API filter and again on each parsed `created_at`; removing it makes a stale run reappear and refuses the claim `DISPATCH_CORRELATION_AMBIGUOUS`.

...
4. the live canonical control ref for the active owning system
5. the sealed checkpoint, completion ledger and current obligation registry selected by CURRENT-AUTHORITY
6. expectation/reallocation/repair state relevant to the active owner
7. the registry-declared Second Shift delegation and current shift ledger when unattended work is involved

Conversation memory, morning packets, branch names and historical handoffs never override current repository authority.
...
Historical Prose or Document PASS never transfers to changed Documents integration subjects.
...
`LIVE LOOKUP -> OWNER/TOPOLOGY RESOLUTION -> DELTA CLASSIFICATION -> RECONCILE -> CURRENT STATE -> NEXT STEP`

Do not ask the user to reconstruct architecture or history that current authority can resolve. A new chat is a new working surface, not a new project state.

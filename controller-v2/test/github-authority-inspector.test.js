...
function config(){return {mode:'authoritative',subjectRepository:'BFochtman746/system-master',controlStateRepository:'BFochtman746/system-master-control-state',journal:{branch:'controller-journal/v1',principal:{kind:'github-app',id:101}},anchor:{branch:'controller-anchor/v1',principal:{kind:'github-app',id:202}}};}
...
function setup({missingRepo=false,hideBypass=false}={}){
  ...
  const fetchImpl=async(url,options)=>{
    ...
    if(path==='/repos/BFochtman746/system-master-control-state')return missingRepo?response(404,{message:'Not Found'}):response(200,{full_name:'BFochtman746/system-master-control-state'});
...
test('GAI-T001 privileged inspector resolves effective rule IDs to full bypass-aware rulesets',async()=>{
  const observation=await setup().inspect({journalBranch:'controller-journal/v1',anchorBranch:'controller-anchor/v1'});
  assert.equal(observation.repository_exists,true);assert.equal(observation.journal.writer_rulesets[0].id,11);assert.equal(observation.journal.integrity_rulesets[0].id,12);assert.equal(observation.anchor.writer_rulesets[0].id,21);assert.equal(observation.anchor.integrity_rulesets[0].id,22);
  assert.equal(assertAuthoritativePreflight(config(),observation).authoritative,true);
});

test('GAI-T002 missing repository is reported without inventing protection state',async()=>{const o=await setup({missingRepo:true}).inspect({journalBranch:'controller-journal/v1',anchorBranch:'controller-anchor/v1'});assert.equal(o.repository_exists,false);assert.equal(o.journal,null);assert.equal(o.anchor,null);});

...
test('GHJ-T007 stale transport revision is rejected',async()=>{
  const {journal}=setup();await journal.putIfAbsent(event());
  const old=(await journal.getCheckpoint()).transport_revision;await journal.putIfAbsent(event({stream:'b'}));
...
test('GHJ-T005/T008 two writers from one parent produce one winner and loser objects remain unreachable',async()=>{
  ...
  const settled=await Promise.allSettled([journal.appendBatch([event({stream:'a2'})]),j2.appendBatch([event({stream:'b2'})])]);transport.beforeRefUpdate=null;
  ...
  assert.equal((await journal.list()).length,2);assert.ok(transport.commits.size>=4);
});
...
test('GHJ-T017/T018 missing predecessor batch breaks chain and intact journal recreates exact 002B checkpoint',async()=>{
  ...
  transport.deleteFileAtRef('controller-journal/v1',cp1.last_batch_path);await assertRejectCode(journal.list(),'JOURNAL_BATCH_MISSING');
});
...
test('GHJ-T020 transport outage before ref advance leaves SQLite outbox unsealed',async()=>{
  ...
  const command={protocol_version:'1.0',schema:'controller://schemas/command/v1',command_id:uuidv7(),created_at:new Date().toISOString(),issuer:{principal:'user:test',source:'chatgpt'},command_type:'controller.work.submit',target:{repository:'BFochtman746/system-master',expected_subject:{algorithm:'sha1',oid:'0'.repeat(40)}},preconditions:{},intent:{task:'journal-outage'},constraints:{},required_policy_version:null};

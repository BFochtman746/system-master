...
function observed(){return {repository_exists:true,repository_full_name:'BFochtman746/system-master-control-state',journal:{branch_exists:true,writer_rulesets:[writer(101)],integrity_rulesets:[integrity()]},anchor:{branch_exists:true,writer_rulesets:[writer(202)],integrity_rulesets:[integrity()]}};}
function assertCode(fn,code){assert.throws(fn,e=>e instanceof ControllerError&&e.code===code);}

test('APF-T001 exact separate-repo/separate-app/layered-ruleset topology passes',()=>{const r=assertAuthoritativePreflight(CONFIG,observed());assert.equal(r.authoritative,true);assert.equal(r.controlStateRepository,'bfochtman746/system-master-control-state');});
...
test('APF-T006 wrong observed repository identity blocks activation',()=>{const o=observed();o.repository_full_name='BFochtman746/system-master';assertCode(()=>assertAuthoritativePreflight(CONFIG,o),'AUTH_REPOSITORY_IDENTITY_MISMATCH');});

...
test('SM-T001 backup is independently integrity-verified', async()=>{
  const {dir,db,backup}=tempPaths();
  try{
    const k=new ControllerKernel(db);k.acceptCommand(command());
    const result=await backupControllerStore(k,backup);
...
test('SM-T002 backup is a point-in-time copy and does not mutate with source', async()=>{
  const {dir,db,backup}=tempPaths();
  try{
    const k=new ControllerKernel(db);k.acceptCommand(command('before'));
    await backupControllerStore(k,backup);
    k.acceptCommand(command('after'));
    assert.equal(k.db.prepare('SELECT COUNT(*) n FROM transactions').get().n,2);
    const b=new DatabaseSync(backup);
    assert.equal(b.prepare('SELECT COUNT(*) n FROM transactions').get().n,1);
    b.close();k.close();
  }finally{rmSync(dir,{recursive:true,force:true});}
});

test('SM-T003 verified backup remains usable after source files are lost', async()=>{
  const {dir,db,backup}=tempPaths();
  try{
    const k=new ControllerKernel(db);const tx=k.acceptCommand(command('recover')).transaction_id;
    await backupControllerStore(k,backup);k.close();
    for(const path of [db,`${db}-wal`,`${db}-shm`]) if(existsSync(path)) unlinkSync(path);
    const recovered=new ControllerKernel(backup);
    assert.equal(recovered.db.prepare('SELECT transaction_id FROM transactions').get().transaction_id,tx);

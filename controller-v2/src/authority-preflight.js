...
function positiveId(value,label){const n=Number(value);if(!Number.isSafeInteger(n)||n<=0||String(value).trim()==='')throw new ControllerError('AUTH_CONFIG_INVALID',`${label} must be a positive integer`);return String(n);}
...
function principal(value,label){if(!value||value.kind!=='github-app')throw new ControllerError('AUTH_CONFIG_INVALID',`${label} must identify a GitHub App`);return {kind:'github-app',id:positiveId(value.id,`${label}.id`)};}
...
export function validateAuthorityConfiguration(config){
  ...
  const controlStateRepositoryId=positiveId(config.controlStateRepositoryId,'controlStateRepositoryId');
  ...
  return {subjectRepository,controlStateRepository,controlStateRepositoryId,journalBranch,anchorBranch,journalPrincipal,anchorPrincipal};
}
...
function validateBranchLayers(label,layers,expectedApp){
  ...
  const authorizedWriter=writer.find(r=>{const actors=bypass(r);return actors.length===1&&appMatches(actors[0],expectedApp);});
  ...
  for(const r of integrity)for(const actor of bypass(r))if(actor.bypass_mode==='always')throw new ControllerError('AUTH_INTEGRITY_BYPASS_PRESENT',`${label} integrity rulesets may not contain always-bypass actors`);
  return true;
}
...
export function assertAuthoritativePreflight(config,observation){
  ...
  if(positiveId(observation.repository_id,'observation.repository_id')!==normalized.controlStateRepositoryId)throw new ControllerError('AUTH_REPOSITORY_IDENTITY_MISMATCH','observed numeric repository ID does not match configured control-state repository');

...
function manifestDigest(manifestWithoutDigest) {
  return sha256(manifestWithoutDigest);
}
...
export class GitHubDurableJournal {
  ...
  async readStreamHead(revision, streamId) {
    ...
    if (value.stream_id !== streamId || !Number.isSafeInteger(value.stream_version) || value.stream_version < 1 || !DIGEST_RE.test(value.event_digest ?? '')) {
      throw new ControllerError('JOURNAL_STREAM_HEAD_INVALID', 'stored stream head is malformed');
    }
    return value;
  }
  ...
  async appendBatch(events, { expectedCheckpoint = null, expectedTransportRevision = undefined } = {}) {
    ...
    for (const event of events) {
      ...
      if (stored.event_digest !== event.event_digest || canonicalize(semanticCore(stored)) !== canonicalize(semanticCore(event))) {
        throw new ControllerError('JOURNAL_CONFLICT', `event id ${event.event_id} already exists with different semantic bytes`);
      }
    ...
    if (existing.every(Boolean)) {
      return {
        created: false,
        events: existing.map((e) => ({ event_id:e.event_id,event_digest:e.event_digest,journal_position:e.journal_position,journal_digest:e.journal_digest })),
        checkpoint: head.checkpoint,
        transport_revision: head.revision
      };
    }
    ...
    for (const streamId of new Set(events.map((e) => e.stream_id))) {
      streamHeads.set(streamId, await this.readStreamHead(head.revision, streamId));
    }
    ...
    for (const [streamId, streamHead] of streamHeads.entries()) {
      if (events.some((e) => e.stream_id === streamId)) files[streamPath(streamId)] = canonicalize(streamHead);
    }

    const expectedCore = normalizeCheckpoint(head.checkpoint);
    const resultCore = { size: position, head_digest: globalHead };
    const batchSeed = {
      protocol_version:JOURNAL_PROTOCOL,
      previous_batch_path:head.checkpoint.last_batch_path,
      expected_checkpoint:expectedCore,
      result_checkpoint:resultCore,
      events:storedEvents.map((e) => ({event_id:e.event_id,event_path:eventPath(e.event_id),event_digest:e.event_digest,journal_position:e.journal_position,journal_digest:e.journal_digest}))
    };
    ...
    const commit = await this.transport.createCommitFromFiles({
      parentSha: head.parent_sha,
      files,
      message: `controller journal: ${expectedCore.size + 1}-${resultCore.size} ${batch_id.slice(0,12)}`
    });

    try {
      if (head.exists) await this.transport.updateRefFastForward(this.branch, commit.sha);
      else await this.transport.createRef(this.branch, commit.sha);
    } catch (error) {
      if (isHeadConflict(error)) throw new ControllerError('JOURNAL_HEAD_CONFLICT', 'journal ref changed during append');
      // A network failure can happen after the ref was advanced. Observe before declaring failure.
    ...
    return {
      created:true,
      events:storedEvents.map((e) => ({event_id:e.event_id,event_digest:e.event_digest,journal_position:e.journal_position,journal_digest:e.journal_digest})),
      checkpoint,
      transport_revision:commit.sha,
      batch_path:bPath,
      batch_digest
    };
  }
  ...
  async readManifest(revision, path, expectedDigest = null) {
    ...
    if (batchPath(value.batch_id) !== path) throw new ControllerError('JOURNAL_BATCH_INTEGRITY_FAILURE', 'batch id/path mismatch');
    return value;
  }

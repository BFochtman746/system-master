...
export function verifyAndGroupEvents(events) {
  ...
  for (const event of events) {
    if (event.event_schema !== EVENT_SCHEMA) {
      throw new ControllerError('UNSUPPORTED_EVENT_SCHEMA', `unsupported historic event schema ${event.event_schema}`);
    }
  ...
  for (const list of grouped.values()) {
    ...
    for (const event of list) {
      ...
      if (event.prev_event_digest !== previousDigest || sha256(core) !== event.event_digest) {
        throw new ControllerError('EVENT_INTEGRITY_FAILURE', 'event chain invalid');
      }
    ...
    for (const event of list) {
      ...
      switch (event.event_type) {
        ...
        case 'operation.planned':
          state.operations[d.operation_id] = {
        ...
        case 'qualification.cancelled': {
          ...
          state.qualifications[d.qualification_id] = {
            ...
            state: d.state || event.event_type.slice('qualification.'.length).toUpperCase(),
        ...
        case 'promotion.reconciliation-required': {
          ...
          state.promotions[d.promotion_id] = {
            ...
            state: d.state || event.event_type.slice('promotion.'.length).replaceAll('-', '_').toUpperCase(),

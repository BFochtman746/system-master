...
export function verifyAndGroupEvents(events) {
  ...
  for (const list of grouped.values()) {
    ...
    let previousDigest=null,version=0;
    for (const event of list) {
      ...
      previousDigest=event.event_digest;version+=1;
    }
  }
  return grouped;
}
  ...
  for(const list of grouped.values()) for(const event of list){
    ...
    switch(event.event_type){
      case 'command.accepted':{
        const c=d.command;if(!c?.target?.expected_subject) throw new ControllerError('RECOVERY_PAYLOAD_INVALID','command.accepted missing expected SubjectRef');
        const subject=normalizeSubjectRef(c.target.expected_subject);
        state.commands[d.command_id]={command:c,fingerprint:d.fingerprint,created_at:c.created_at};
        state.transactions[d.transaction_id]={transaction_id:d.transaction_id,command_id:d.command_id,state:'OPEN',controller_version:d.controller_version,policy_version:d.policy_version,subject_repo:c.target.repository,subject,completion_contract:null,created_at:event.occurred_at,updated_at:event.occurred_at};
        break;
      }
      case 'transaction.admitted':case 'transaction.active':case 'transaction.waiting':case 'transaction.succeeded':case 'transaction.rejected':case 'transaction.failed':case 'transaction.cancelled':case 'transaction.superseded':{
        const tx=state.transactions[d.transaction_id];if(!tx) throw new ControllerError('RECOVERY_REFERENCE_MISSING',`transaction event precedes command.accepted for ${d.transaction_id}`);
      ...
      case 'lease.granted':if(!d.resource_id||!Number.isInteger(Number(d.generation)))throw new ControllerError('RECOVERY_PAYLOAD_INVALID','lease.granted missing resource/generation');state.resource_generations[d.resource_id]=Math.max(state.resource_generations[d.resource_id]||0,Number(d.generation));break;
      case 'qualification.requested':case 'qualification.running':case 'qualification.passed':case 'qualification.failed':case 'qualification.indeterminate':case 'qualification.cancelled':{
        const prior=state.qualifications[d.qualification_id],subject=d.subject?normalizeSubjectRef(d.subject):prior?.subject;if(!subject)throw new ControllerError('RECOVERY_PAYLOAD_INVALID','qualification event missing SubjectRef');
        state.qualifications[d.qualification_id]={qualification_id:d.qualification_id,transaction_id:d.transaction_id??prior?.transaction_id,subject,policy_version:d.policy_version??prior?.policy_version,state:d.state||event.event_type.slice('qualification.'.length).toUpperCase(),created_at:prior?.created_at??event.occurred_at,updated_at:event.occurred_at};break;
      }

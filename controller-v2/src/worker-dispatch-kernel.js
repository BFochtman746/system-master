...
function rowCapabilities(row) { return JSON.parse(row.capabilities_json); }

function dispatchIdentity(row) {
  return {
    protocol: WORKER_DISPATCH_PROTOCOL,
...
export class WorkerDispatchKernel extends ExecutionGraphKernel {
  ...
  recordWorkerBinding(receipt, createdAt = new Date().toISOString()) {
    ...
    if (Date.parse(semantic.valid_until) <= Date.parse(semantic.observed_at)) throw new ControllerError('WORKER_BINDING_VALIDITY_INVALID', 'valid_until must be after observed_at');
    const bindingId = sha256(semantic);
    return this.atomic(() => {
      const prior = this.db.prepare('SELECT * FROM worker_bindings WHERE binding_id=?').get(bindingId);
  ...
  revokeWorkerBinding(bindingId, revocation) {
    ...
    return this.atomic(() => {
      ...
      this.db.prepare('INSERT INTO worker_binding_revocations(binding_id,authority,evidence_ref,evidence_digest,observed_at) VALUES (?,?,?,?,?)').run(bindingId,normalized.authority,normalized.evidence_ref,normalized.evidence_digest,normalized.observed_at);
  ...
  _dispatchRows(dispatchId) {
    return this.db.prepare(`SELECT d.*,c.executor_kind,c.required_capabilities_json,c.payload_ref,c.payload_digest,b.worker_ref,b.capabilities_json,b.observed_at,b.valid_until
      FROM dispatch_intents d JOIN execution_contracts c ON c.contract_id=d.contract_id JOIN worker_bindings b ON b.binding_id=d.worker_binding_id WHERE d.dispatch_id=?`).get(dispatchId);
  }
  ...
  createDispatchIntent({ operationId, contractId, workerBindingId, leaseId, nowMs = Date.now() }) {
    return this.atomic(() => {
      const op = this.db.prepare(`SELECT o.*,t.state transaction_state FROM operations o JOIN transactions t ON t.transaction_id=o.transaction_id WHERE o.operation_id=?`).get(operationId);
      ...
      if (!contract || contract.operation_id !== operationId || contract.transaction_id !== op.transaction_id || contract.resource_id !== op.resource_id) throw new ControllerError('DISPATCH_CONTRACT_MISMATCH', 'execution contract does not match operation');
      ...
      const check = { worker_binding_id: workerBindingId, required_capabilities_json: contract.required_capabilities_json, capabilities_json: binding.capabilities_json };
      this._assertDispatchBindingCurrent(check, nowMs);
      ...
      const envelope = { protocol:WORKER_DISPATCH_ENVELOPE_PROTOCOL,dispatch_id:dispatchId,...identity };
      delete envelope.protocol;
      const canonicalEnvelope = { protocol:WORKER_DISPATCH_ENVELOPE_PROTOCOL,dispatch_id:dispatchId,transaction_id:op.transaction_id,operation_id:operationId,contract_id:contractId,worker_binding_id:workerBindingId,lease_id:leaseId,resource_id:lease.resource_id,generation:Number(lease.generation) };
      const requestDigest = sha256(canonicalEnvelope);
  ...
  prepareWorkerDispatchEffect(dispatchId, nowMs = Date.now()) {
    ...
    const result = prepareExternalEffect(this, {
      transaction_id: row.transaction_id,
      operation_id: row.operation_id,
      provider: `worker-transport:${row.executor_kind}`,
  ...
  activateWorkerDispatch(dispatchId, effectId, nowMs = Date.now()) {
    ...
    if (effect.transaction_id !== row.transaction_id || effect.operation_id !== row.operation_id || effect.idempotency_key !== row.dispatch_id || effect.request_digest !== row.request_digest || effect.effect_type !== 'controller.worker.dispatch') throw new ControllerError('DISPATCH_EFFECT_MISMATCH', 'effect does not match dispatch intent');
  ...
  getWorkerDispatchPermit(dispatchId, effectId, nowMs = Date.now()) {
    ...
    this._assertDispatchBindingCurrent(row, nowMs);
    const effect = getExternalEffect(this, effectId);
    if (effect.idempotency_key !== row.dispatch_id || effect.request_digest !== row.request_digest) throw new ControllerError('DISPATCH_EFFECT_MISMATCH', 'effect does not match dispatch intent');
    const envelope = workerDispatchEnvelope(row);
    if (sha256(envelope) !== row.request_digest) throw new ControllerError('DISPATCH_REQUEST_DIGEST_MISMATCH', 'dispatch envelope differs from durable intent');
  ...
  restoreWorkerDispatchProjection(events = this.exportEvents()) {
    ...
    return this.atomic(() => {
      ...
      for (const event of bindings) {
        const d=event.data??{};
        const semantic={protocol:d.protocol,worker_ref:d.worker_ref,identity_authority:d.identity_authority,identity_evidence_ref:d.identity_evidence_ref,identity_evidence_digest:d.identity_evidence_digest,delegation_evidence_ref:d.delegation_evidence_ref,delegation_evidence_digest:d.delegation_evidence_digest,verifier_policy_revision:d.verifier_policy_revision,verifier_policy_digest:d.verifier_policy_digest,capabilities:d.capabilities,capability_digest:d.capability_digest,observed_at:d.observed_at,valid_until:d.valid_until};
        if (semantic.protocol!==WORKER_BINDING_PROTOCOL || sha256(semantic)!==d.binding_id || semantic.capability_digest!==sha256(canonicalCapabilities(semantic.capabilities))) throw new ControllerError('RECOVERY_WORKER_DISPATCH_INVALID','invalid worker binding event');
        this.db.prepare(`INSERT INTO worker_bindings(binding_id,worker_ref,identity_authority,identity_evidence_ref,identity_evidence_digest,delegation_evidence_ref,delegation_evidence_digest,verifier_policy_revision,verifier_policy_digest,capabilities_json,capability_digest,observed_at,valid_until,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
          .run(d.binding_id,d.worker_ref,d.identity_authority,d.identity_evidence_ref,d.identity_evidence_digest,d.delegation_evidence_ref,d.delegation_evidence_digest,d.verifier_policy_revision,d.verifier_policy_digest,canonicalize(d.capabilities),d.capability_digest,d.observed_at,d.valid_until,event.occurred_at);
      }
      for (const event of revocations) {
        ...
        this.db.prepare('INSERT INTO worker_binding_revocations(binding_id,authority,evidence_ref,evidence_digest,observed_at) VALUES (?,?,?,?,?)').run(d.binding_id,d.authority,d.evidence_ref,d.evidence_digest,d.observed_at);
      }
      for (const event of contracts) {
        const d=event.data??{};
        const semantic={protocol:d.protocol,transaction_id:d.transaction_id,operation_id:d.operation_id,subject_repo:d.subject_repo,subject_algorithm:d.subject_algorithm,subject_oid:d.subject_oid,resource_id:d.resource_id,executor_kind:d.executor_kind,protocol_version:d.protocol_version,payload_ref:d.payload_ref,payload_digest:d.payload_digest,required_capabilities:d.required_capabilities,required_capability_digest:d.required_capability_digest};
        if (semantic.protocol!==EXECUTION_CONTRACT_PROTOCOL || sha256(semantic)!==d.contract_id || semantic.required_capability_digest!==sha256(canonicalCapabilities(semantic.required_capabilities))) throw new ControllerError('RECOVERY_WORKER_DISPATCH_INVALID','invalid execution contract event');
        const op=this.db.prepare('SELECT transaction_id,resource_id FROM operations WHERE operation_id=?').get(d.operation_id);
        if (!op || op.transaction_id!==d.transaction_id || op.resource_id!==d.resource_id) throw new ControllerError('RECOVERY_WORKER_DISPATCH_INVALID','contract operation binding mismatch');
        this.db.prepare(`INSERT INTO execution_contracts(contract_id,transaction_id,operation_id,subject_repo,subject_algorithm,subject_oid,resource_id,executor_kind,protocol_version,payload_ref,payload_digest,required_capabilities_json,required_capability_digest,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
          .run(d.contract_id,d.transaction_id,d.operation_id,d.subject_repo,d.subject_algorithm,d.subject_oid,d.resource_id,d.executor_kind,d.protocol_version,d.payload_ref,d.payload_digest,canonicalize(d.required_capabilities),d.required_capability_digest,event.occurred_at);
      }
      for (const event of dispatches) {
        const d=event.data??{};
        const identity={protocol:WORKER_DISPATCH_PROTOCOL,transaction_id:d.transaction_id,operation_id:d.operation_id,contract_id:d.contract_id,worker_binding_id:d.worker_binding_id,lease_id:d.lease_id,resource_id:d.resource_id,generation:Number(d.generation)};
        const dispatchId=sha256(identity);
        const envelope={protocol:WORKER_DISPATCH_ENVELOPE_PROTOCOL,dispatch_id:dispatchId,transaction_id:d.transaction_id,operation_id:d.operation_id,contract_id:d.contract_id,worker_binding_id:d.worker_binding_id,lease_id:d.lease_id,resource_id:d.resource_id,generation:Number(d.generation)};

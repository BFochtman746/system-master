...
import { closeSync, existsSync, fsyncSync, openSync, readFileSync, renameSync, statSync, unlinkSync } from 'node:fs';
...
function removeIfPresent(path) {
  try { if (existsSync(path)) unlinkSync(path); } catch { /* best-effort cleanup; original failure wins */ }
}
...
export async function backupControllerStore(kernel, destination, { rate = 64 } = {}) {
  ...
  if (typeof destination !== 'string' || destination.length === 0) throw new ControllerError('BACKUP_DESTINATION_INVALID', 'backup destination required');
  if (existsSync(destination)) throw new ControllerError('BACKUP_DESTINATION_EXISTS', 'backup destination must not already exist; use a new immutable backup path');
  ...
  try {
    ...
    return { pages, integrity, schema_version: schemaVersion, events, transactions, bytes: evidence.bytes, sha256: evidence.sha256, destination };
  } catch (error) {

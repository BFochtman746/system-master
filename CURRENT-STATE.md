...
| Document-truth ratchet | `.github/scripts/document-truth-ratchet-qualify.py`, `qualification/document-truth/ratchet-baseline.json` | **Landed 2026-09-18.** Extends the #243 gate from 2 documents to all 164 as a per-file baseline that can only fall. Measured: 674 path assertions, **99 contradicted across 42 documents** — 73 live repair backlog, 26 frozen historical records. New fiction in any document fails immediately; a ceiling above measured reality fails as stale, forcing the fixing diff to tighten it; a raised ceiling is refused against the baseline as committed at the base ref, so the gate cannot authorize its own weakening. 13 self-proofs including a vacuous-walk floor of 120 documents. Emits a machine-consumable violation list (document, line, asserted path) as the first autonomous-repair workload. Suite count 42 → 43. |
| Doc-truth resolution is build-independent | `.github/scripts/document-truth-qualify.py` | **Fixed 2026-09-18.** The gate resolved a document's file claim with `os.path.exists`, so gitignored generated output could satisfy it: the same commit measured 94 violations in a built tree and 99 in a clean one, because `qualification-output/` answered a contract document's claim. Claims now resolve against git's view — tracked plus untracked-not-ignored — which excludes `target/` and `qualification-output/` deterministically while still seeing a new source file committed alongside the document describing it. Proved by a negative self-proof and by measuring both tree states. A baseline built on the old measurement would have been worthless. |

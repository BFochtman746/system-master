...
class T(unittest.TestCase):
 ...
 def test_workflow_guardrails(self):
  x=(ROOT/".github"/"workflows"/"control-gateway-native-a01-run-now-ingress.yml").read_text();self.assertIn("types: [opened]",x);self.assertIn("github.actor == github.repository_owner",x);self.assertIn("author_association == 'OWNER'",x);self.assertIn("a01-global-r2",x);self.assertIn("Add-ExistingSupervisorDb",x);self.assertIn("SystemMaster-A01-Execution",x);self.assertIn(r"C:\SystemMaster\a01-supervisor.db",x);self.assertIn("Test-Path -LiteralPath $full -PathType Leaf",x);self.assertIn("A01_SUPERVISOR_DB_AMBIGUOUS",x);self.assertIn("if($e.Count -eq 1)",x);self.assertIn("New-Item -ItemType Directory -Force",x);self.assertIn("git config --global --add safe.directory $env:GITHUB_WORKSPACE",x);self.assertIn("actions: write",x);self.assertIn("A01_REPAIR_DISPATCH_TOKEN: ${{ github.token }}",x);self.assertNotIn("safe.directory '*'",x);self.assertNotIn('safe.directory "*"',x);self.assertNotIn("A01_SUPERVISOR_DB_UNRESOLVED",x);self.assertNotIn("contents: write",x)

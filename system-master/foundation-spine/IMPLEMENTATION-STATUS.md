...
| Contracts & Versioning | CURRENT — HOSTED QUALIFIED | Fresh Contracts & Versioning authority now has a 30/30 bounded invariant census with zero unaccounted rows, a durable hosted-portable reference registry, exact subject/version/digest/canonicalizer identity, explicit directional/transitive validator-bound compatibility, immutable migration edges, separate lifecycle standing, fail-closed journal replay, command replay/conflict handling and a structural pre-mutation gate that cannot grant foreign authority. Exact candidate `777a2937f446ca2b28424915da5034fc3d04f2e1` passed the frozen 48-case isolated denominator plus cumulative System Root/O-WP-001/O-WP-002/Identity Delegation qualification and the recovered F-WP-012 47-case donor regression. Its tree `b9bd93bd1ba56997f3fc379794d5f574a0145fae` is identical to live merge `d175cc72fe86d47386593e7545e4027bce53bfa0`. Real format-validator semantics, A-01, native sudden-power-loss, distributed/network-filesystem writer safety, production storage and production admission remain separate evidence boundaries. |
| Intent / Keel | HISTORICAL SUBSTRATE — REBIND / DESIGN LOCKED | Recovery now accounts 32/32 bounded Keel invariants with zero unaccounted rows and adjudicates Keel as the owner of immutable governed intent/revisions, requirements/constraints, success declarations and delegation/resource/action/HITL ceilings only. Concrete Identity grants, Resource grants/accounting, Work/Project truth, plans, routing/placement, runtime state, effects, human evidence and specialist semantics remain outside Keel. `KEEL-DESIGN-LOCK-001.md` freezes the fresh object model, parent->child non-expansion rules, durable publication semantics, `GovernedGoalRefV1`, `KeelValidationReceiptV1`, error taxonomy and a 56-case isolated denominator. Runtime standing remains unadvanced until `CORE-KEEL-BUILD-001` is implemented and cumulatively qualified. |
...
| Durable Execution Runtime | HISTORICAL SUBSTRATE — REBIND | Strong JDBC/durable jobs, attempts, leases/fencing, checkpoints, timers/signals, retry/cancel/idempotency/outbox/recovery substrate exists. Preserve it; rebind it as the sole low-level execution truth and harden it for long-duration workloads. |
...
- Contracts & Versioning: `contracts/CONTRACTS-VERSIONING-RECOVERY-INVENTORY-001.md`, `contracts/CONTRACTS-VERSIONING-DESIGN-LOCK-001.md`, `contracts/CONTRACTS-BUILD-TRACEABILITY.json` and `contracts/CONTRACTS-VERSIONING-FREEZE-001.md`; the traceability accounts 30/30 bounded invariants with zero unaccounted rows and the build qualified against a frozen 48-case isolated denominator plus cumulative Foundation regression.
- Intent / Keel: `keel/KEEL-RECOVERY-INVENTORY-001.md` accounts 32/32 bounded invariants with zero unaccounted rows; `keel/KEEL-DESIGN-LOCK-001.md` freezes the ownership/model/refinement/durability/receipt boundary and a 56-case isolated denominator before runtime build.
...
1. **Build and qualify Intent / Keel from the frozen design.** Implement durable governed goal identity/revisions, non-expanding constraint/delegation/resource/action/HITL envelopes and exact downstream governed-goal references without creating foreign authority; then execute the 56-case isolated denominator plus cumulative Root/Identity/Contracts regression.
...
3. **Rebind Work/Project Control and Orchestrator.** Preserve long-lived work/project semantics, repair Orchestrator persistence/lifecycle inconsistencies, and make plan execution reference the durable runtime rather than duplicate job truth.

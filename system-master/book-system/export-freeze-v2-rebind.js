...
function ledgerDigest(ledger) { return core.sha256(ledger); }

function createFreezeSpecialistState(canonicalParentInput) {
  const parent = clone(canonicalParentInput);
  core.validateParent(parent);
  const state = {
    schema_version: SPECIALIST_SCHEMA_VERSION,
    book_project_id: parent.book_project.book_project_id,
    canonical_parent_state_version: parent.state_version,
    canonical_parent_state_digest: parent.state_digest,
    ledger_version: 1,
    processed_requests: {},
    freeze_receipts: {},
  };
  validateFreezeSpecialistState(state, parent);
  return state;
}

function validateFreezeSpecialistState(stateInput, canonicalParentInput) {
  ...
  if (!Number.isInteger(state.ledger_version) || state.ledger_version < 1) fail('INVALID_FREEZE_LEDGER_VERSION');
  if (!obj(state.processed_requests) || !obj(state.freeze_receipts)) fail('INVALID_FREEZE_LEDGER_INDEX');
  return true;
}

function exportFreezeIdentity(stateInput, canonicalParentInput) {
  validateFreezeSpecialistState(stateInput, canonicalParentInput);
...
function validateAdmissions(parent, refs) {
  ...
  for (const ref of refs) {
    ...
    seen.add(ref);
    const proposal = (parent.integration_proposals || []).find(p => p && p.proposal_id === ref);
    if (!proposal || proposal.admission_state !== 'ADMITTED') fail('PARENT_ADMISSION_NOT_ADMITTED', ref);
  }
}
...
function validateAuthorDecisions(parent, refs) {
  ...
  for (const ref of refs) {
    ...
    seen.add(ref);
    const decision = (parent.author_decisions || []).find(d => d && d.decision_id === ref);
    if (!decision || decision.status !== 'APPROVED') fail('AUTHOR_DECISION_NOT_CURRENT_APPROVED', ref);
  }
}
...
function prepareExportFreezeRegistration({canonicalParent:parentInput, exportFreezeState:stateInput, request:requestInput}) {
  const parent=clone(parentInput), state=clone(stateInput), request=clone(requestInput);
  core.validateParent(parent);
  const preIdentity=validateRequest(parent,state,request);
  const prior=state.processed_requests[request.idempotency_key];
  const fingerprint=core.sha256(request);
  if (prior) {
    if (prior.request_fingerprint !== fingerprint) fail('IDEMPOTENCY_KEY_CONFLICT');
    fail('ALREADY_COMMITTED_REQUIRES_RECEIPT_RECONCILIATION');
  }
  const manuscript=activeManuscript(parent,request.canonical_manuscript_identity);
  validateEvidence(request,manuscript);
  validateAdmissions(parent,request.parent_admission_refs);
  validateAuthorDecisions(parent,request.author_decision_refs);
  ...
  const release={
    ...
    book_project_id:parent.book_project.book_project_id,
    frozen_version_id:String(manuscript.record.version),
    format:request.artifact_evidence.format_identity,
    digest:request.artifact_evidence.artifact_digest,
    approval_state:'FROZEN',
    publication_authority_state:'NOT_AUTHORIZED',
    publication_authorized:false,
    evidence_type:'EXPORT_FREEZE_READY',
    canonical_manuscript_id:manuscript.record.object_id,
    canonical_manuscript_version_id:String(manuscript.record.version),
    canonical_manuscript_digest:manuscript.digest,
    artifact_ref:request.artifact_evidence.artifact_ref,
    artifact_currentness:request.artifact_evidence.artifact_currentness,
    ...
    technical_state:request.target_medium_evidence.technical_state,
    format_validation_result:request.target_medium_evidence.format_validation_result,
    source_to_render_integrity_result:request.target_medium_evidence.source_to_render_integrity_result,
    accessibility_result:request.target_medium_evidence.accessibility_result,
    proof_result:request.target_medium_evidence.proof_result,
    correction_ledger_ref:request.target_medium_evidence.correction_ledger_ref,
    rights_privacy_evidence_ref:request.rights_privacy_evidence.evidence_ref,
    provenance_projection_ref:request.provenance_evidence.projection_ref,
    provenance_currentness_state:request.provenance_evidence.currentness_state,
  ...
  const effect={
    ...
    effect_request_id:`BEF-PARENT-${core.sha256({freeze_request_id:request.freeze_request_id,idempotency_key:request.idempotency_key}).slice(0,32).toUpperCase()}`,
    idempotency_key:`BEF:${request.idempotency_key}`,
    actor_class:'SYSTEM', authority_ref:request.authority_ref,
    expected_parent_state_version:parent.state_version, expected_parent_state_digest:parent.state_digest,
    effect_type:'REGISTER_EXPORT_RELEASE', effect_payload:payload, effect_payload_digest:core.effectPayloadDigest(payload),
    subject_identity_refs:uniq([`${manuscript.record.object_id}:${String(manuscript.record.version)}:${manuscript.digest}`, request.release_id]),
    evidence_refs:uniq([request.artifact_evidence.artifact_ref,request.rights_privacy_evidence.evidence_ref,request.provenance_evidence.projection_ref,...request.parent_admission_refs,...request.author_decision_refs]),
    specialist_receipt_refs:[receiptId], created_at:request.created_at, expected_specialist_ledger_identity:preIdentity,
  };
  ...
  next.canonical_parent_state_version=anticipated.state_version;
  next.canonical_parent_state_digest=anticipated.state_digest;
  ...
  const receipt={
    receipt_schema_version:SPECIALIST_RECEIPT_SCHEMA_VERSION,receipt_id:receiptId,specialist_kind:SPECIALIST_KIND,
    book_project_id:parent.book_project.book_project_id,freeze_request_id:request.freeze_request_id,idempotency_key:request.idempotency_key,request_fingerprint:fingerprint,
    release_id:request.release_id,pre_export_freeze_identity:preIdentity,post_export_freeze_identity:'__PENDING__',
    pre_parent_state_version:parent.state_version,pre_parent_state_digest:parent.state_digest,post_parent_state_version:anticipated.state_version,post_parent_state_digest:anticipated.state_digest,
    publication_authority_state:'NOT_AUTHORIZED',disposition:'PREPARED_FOR_PARENT',created_at:request.created_at,
  };
  next.freeze_receipts[receiptId]=receipt;
  receipt.post_export_freeze_identity=core.sha256(next);
  next.freeze_receipts[receiptId]=receipt;
  const postIdentity=core.sha256(next);
  receipt.post_export_freeze_identity=postIdentity;
  receipt.receipt_digest=core.sha256(receipt);
  next.freeze_receipts[receiptId]=receipt;
  const finalIdentity=core.sha256(next);
  receipt.post_export_freeze_identity=finalIdentity;
  delete receipt.receipt_digest;
  receipt.receipt_digest=core.sha256(receipt);
  next.freeze_receipts[receiptId]=receipt;
  const canonicalIdentity=core.sha256(next);
  if (canonicalIdentity !== receipt.post_export_freeze_identity) {
    receipt.post_export_freeze_identity=canonicalIdentity;
    delete receipt.receipt_digest; receipt.receipt_digest=core.sha256(receipt); next.freeze_receipts[receiptId]=receipt;
  }
  validateFreezeSpecialistState(next,anticipated);
  const delta={specialist_kind:SPECIALIST_KIND,book_project_id:parent.book_project.book_project_id,expected_export_freeze_identity:preIdentity,post_export_freeze_identity:core.sha256(next),post_state:next,specialist_receipt:clone(receipt)};
  delta.specialist_receipt.post_export_freeze_identity=delta.post_export_freeze_identity;
  delete delta.specialist_receipt.receipt_digest; delta.specialist_receipt.receipt_digest=core.sha256(delta.specialist_receipt);
  delta.post_state.freeze_receipts[receiptId]=clone(delta.specialist_receipt);
  delta.post_export_freeze_identity=core.sha256(delta.post_state);
  delta.specialist_receipt.post_export_freeze_identity=delta.post_export_freeze_identity;
  delete delta.specialist_receipt.receipt_digest; delta.specialist_receipt.receipt_digest=core.sha256(delta.specialist_receipt);
  delta.post_state.freeze_receipts[receiptId]=clone(delta.specialist_receipt);
  delta.post_export_freeze_identity=core.sha256(delta.post_state);
  delta.delta_digest=core.sha256({specialist_kind:delta.specialist_kind,book_project_id:delta.book_project_id,expected_export_freeze_identity:delta.expected_export_freeze_identity,post_export_freeze_identity:delta.post_export_freeze_identity,receipt_id:receiptId,release_id:request.release_id});

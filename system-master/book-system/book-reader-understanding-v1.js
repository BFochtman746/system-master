...
const CANDIDATE_FIELDS = Object.freeze([
  'observation_schema_version','exposure_projection_id','exposure_projection_digest','target','source_class','standing',
  'provider_subject_ref','provider_admission_ref','reader_profile_ref','evidence_refs','confidence','calibration_ref','reveal_frontier','canonical_effect'
]);
const SEALED_FIELDS = Object.freeze([
  'observation_schema_version','observation_id','observation_digest','exposure_projection_id','exposure_projection_digest','target','source_class','standing',
  'provider_subject_ref','provider_admission_ref','provider_admission_digest','reader_profile_ref','evidence_refs','confidence','calibration_ref','calibration_digest','calibration_class',
  'reveal_frontier','canonical_effect'
]);
...
function nullableDigest(v) { return v === null || digest(v); }

function req(v, fields, label) {
  if (!obj(v)) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', `${label}:OBJECT_REQUIRED`);
  for (const field of fields) if (!Object.prototype.hasOwnProperty.call(v, field)) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', `${label}.${field}`);
}

function assertExactKeys(v, fields, label) {
  const expected = new Set(fields);
  for (const key of Object.keys(v)) if (!expected.has(key)) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', `${label}.unexpected:${key}`);
...
function assertNoForbiddenPayload(value, path = '$') {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (/raw.*(?:manuscript|source|document|text|bytes)/i.test(key) || /quoted.*text/i.test(key) || /full.*text/i.test(key) || /chain.*of.*thought/i.test(key) || /hidden.*reason/i.test(key) || /credential/i.test(key) || /secret/i.test(key)) fail('BLOCKED_RAW_TEXT_FORBIDDEN', `${path}.${key}`);
    assertNoForbiddenPayload(child, `${path}.${key}`);
  }
}

function validateFrontierBinding(frontier, exposureProjection) {
  if (!obj(frontier) || !text(frontier.frontier_anchor_id) || !Number.isInteger(frontier.frontier_ordinal) || frontier.frontier_ordinal < 0 || !digest(frontier.frontier_anchor_digest)) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', 'frontier');
  const e = exposureProjection.reveal_frontier;
  if (frontier.frontier_anchor_id !== e.frontier_anchor_id || frontier.frontier_ordinal !== e.frontier_ordinal || frontier.frontier_anchor_digest !== e.frontier_anchor_digest) fail('BLOCKED_OBSERVATION_STALE', 'frontier');
}
...
function validateEvidenceRefs(refs, standing) {
  ...
  if ((standing === 'OBSERVED_UNCALIBRATED' || standing === 'OBSERVED_CALIBRATED') && refs.length === 0) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', 'observed_requires_evidence');
}
...
function validateProviderBinding(candidate, providerAdmissionBinding) {
  ...
  if (!text(candidate.provider_subject_ref) || !text(candidate.provider_admission_ref)) fail('BLOCKED_PROVIDER_SUBJECT_UNADMITTED', 'provider_refs_required');
  if (!obj(providerAdmissionBinding) || providerAdmissionBinding.current !== true || providerAdmissionBinding.provider_subject_ref !== candidate.provider_subject_ref || providerAdmissionBinding.provider_admission_ref !== candidate.provider_admission_ref || !digest(providerAdmissionBinding.provider_admission_digest)) fail('BLOCKED_PROVIDER_SUBJECT_UNADMITTED', candidate.provider_subject_ref);
  return providerAdmissionBinding;
}

function validateCalibrationBinding(candidate, calibrationBinding) {
  if (candidate.standing !== 'OBSERVED_CALIBRATED') {
    if (candidate.calibration_ref !== null) fail('BLOCKED_CALIBRATION_EVIDENCE_REQUIRED', 'calibration_ref_without_calibrated_standing');
    return null;
  }
  if (!text(candidate.calibration_ref)) fail('BLOCKED_CALIBRATION_EVIDENCE_REQUIRED', 'calibration_ref');
  if (!obj(calibrationBinding) || calibrationBinding.current !== true || calibrationBinding.calibration_ref !== candidate.calibration_ref || !digest(calibrationBinding.calibration_digest)) fail('BLOCKED_CALIBRATION_EVIDENCE_REQUIRED', candidate.calibration_ref || 'missing');
  ...
  if ((candidate.source_class === 'MODEL' || candidate.source_class === 'SYNTHETIC_READER') && calibrationBinding.calibration_class === 'HUMAN') fail('BLOCKED_HUMAN_STANDING_SYNTHESIZED', candidate.source_class);
  if (calibrationBinding.calibration_class !== candidate.source_class) fail('BLOCKED_CALIBRATION_EVIDENCE_REQUIRED', 'source_class_mismatch');
...
function validateCandidate(candidate, exposureProjection, providerAdmissionBinding, calibrationBinding) {
  ...
  if (candidate.exposure_projection_id !== exposureProjection.exposure_projection_id || candidate.exposure_projection_digest !== exposureProjection.exposure_projection_digest) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', 'exposure');
  ...
  if (!nullableText(candidate.provider_subject_ref) || !nullableText(candidate.provider_admission_ref) || !nullableText(candidate.reader_profile_ref) || !nullableText(candidate.calibration_ref)) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', 'nullable_refs');
  if (candidate.confidence !== null && (typeof candidate.confidence !== 'number' || !Number.isFinite(candidate.confidence) || candidate.confidence < 0 || candidate.confidence > 1)) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', 'confidence');
  ...
  const providerBinding = validateProviderBinding(candidate, providerAdmissionBinding);
  if ((candidate.source_class === 'SYNTHETIC_READER' || candidate.source_class === 'HUMAN') && !text(candidate.reader_profile_ref)) fail(candidate.source_class === 'HUMAN' ? 'BLOCKED_HUMAN_STANDING_SYNTHESIZED' : 'BLOCKED_OBSERVATION_BINDING_MISMATCH', 'reader_profile_ref_required');
  const calibration = validateCalibrationBinding(candidate, calibrationBinding);
  return { providerBinding, calibration };
}
...
function acceptReaderObservationV1(input) {
  ...
  const acceptedBindings = validateCandidate(input.candidate, input.exposure_projection, input.provider_admission_binding || null, input.calibration_binding || null);
  ...
  const output = {
    ...
    provider_admission_digest: acceptedBindings.providerBinding ? acceptedBindings.providerBinding.provider_admission_digest : null,
    reader_profile_ref: c.reader_profile_ref,
    evidence_refs: sorted(c.evidence_refs),
    confidence: c.confidence,
    calibration_ref: c.calibration_ref,
    calibration_digest: acceptedBindings.calibration ? acceptedBindings.calibration.calibration_digest : null,
    calibration_class: acceptedBindings.calibration ? acceptedBindings.calibration.calibration_class : null,
...
function validateSealedProviderAndCalibration(observation) {
  const providerSource = observation.source_class === 'MODEL' || observation.source_class === 'SYNTHETIC_READER';
  if (providerSource) {
    if (!text(observation.provider_subject_ref) || !text(observation.provider_admission_ref) || !digest(observation.provider_admission_digest)) fail('BLOCKED_PROVIDER_SUBJECT_UNADMITTED');
  } else if (observation.provider_subject_ref !== null || observation.provider_admission_ref !== null || observation.provider_admission_digest !== null) {
    if (observation.source_class === 'HUMAN') fail('BLOCKED_HUMAN_STANDING_SYNTHESIZED', 'provider_claim_on_human_source');
    fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', 'provider_binding_not_applicable');
  }

  if (observation.standing === 'OBSERVED_CALIBRATED') {
    if (!text(observation.calibration_ref) || !digest(observation.calibration_digest) || !SOURCE_CLASSES.includes(observation.calibration_class)) fail('BLOCKED_CALIBRATION_EVIDENCE_REQUIRED');
    if ((observation.source_class === 'MODEL' || observation.source_class === 'SYNTHETIC_READER') && observation.calibration_class === 'HUMAN') fail('BLOCKED_HUMAN_STANDING_SYNTHESIZED', observation.source_class);
    if (observation.calibration_class !== observation.source_class) fail('BLOCKED_CALIBRATION_EVIDENCE_REQUIRED', 'source_class_mismatch');
  } else if (observation.calibration_ref !== null || observation.calibration_digest !== null || observation.calibration_class !== null) {
    fail('BLOCKED_CALIBRATION_EVIDENCE_REQUIRED', 'calibration_binding_without_calibrated_standing');
  }
}
...
function validateReaderObservationV1(observation, exposureProjection) {
  ...
  if (!nullableText(observation.provider_subject_ref) || !nullableText(observation.provider_admission_ref) || !nullableDigest(observation.provider_admission_digest) || !nullableText(observation.reader_profile_ref) || !nullableText(observation.calibration_ref) || !nullableDigest(observation.calibration_digest) || (observation.calibration_class !== null && !SOURCE_CLASSES.includes(observation.calibration_class))) fail('BLOCKED_OBSERVATION_BINDING_MISMATCH', 'nullable_bindings');
  ...
  validateSealedProviderAndCalibration(observation);
...
function assembleReaderUnderstandingProjectionV1(input) {
  ...
  const dimensionDispositions = registry.DIMENSIONS.map(d => {
    ...
    return { dimension_id: d.dimension_id, disposition: dispositionFor(matches), observation_refs: matches.map(observationRef) };
  });
  const lensCoverage = registry.LENSES.map(lens => {
    const matches = observations.filter(o => o.target.perspective_lens_id === lens);
    return { perspective_lens_id: lens, disposition: dispositionFor(matches), observation_refs: matches.map(observationRef) };
  });
  ...
  const output = {
    ...
    provider_admission_digests: sorted(observations.map(o => o.provider_admission_digest).filter(Boolean)),
    calibration_refs: sorted(observations.map(o => o.calibration_ref).filter(Boolean)),
    calibration_digests: sorted(observations.map(o => o.calibration_digest).filter(Boolean)),
...
function validateReaderUnderstandingProjectionV1(projection) {
  const required = [
    'understanding_schema_version','understanding_projection_id','understanding_projection_digest','book_project_id','exposure_projection_id','exposure_projection_digest',
    'story_bible_ref','story_bible_digest','knowledge_candidate_id','knowledge_digest','scope','reveal_frontier','observation_refs','dimension_dispositions','lens_coverage',
    'provider_subject_refs','provider_admission_refs','provider_admission_digests','calibration_refs','calibration_digests','external_calibration_fences','standing','canonical_effect'
  ];
  ...
  delete semantic.understanding_projection_id;
  delete semantic.understanding_projection_digest;
  if (hash(semantic) !== projection.understanding_projection_digest) fail('BLOCKED_DIGEST_MISMATCH', 'understanding');
  return true;
}

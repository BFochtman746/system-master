...
class SupervisorStore:
    ...
    def await_evaluation(
        ...
        with self.tx() as c:
            ...
            self._assert_live_worker(c, claim, fencing_token, now)
            outbox = c.execute(
                "SELECT * FROM dispatch_outbox WHERE lease_id=?",
                (lease_id,),
            ).fetchone()
            ...
            c.execute(
                ...
                (iso(now), json.dumps(validation_payload, sort_keys=True), lease_id),
            )
            c.execute(
                "UPDATE delegations SET state='VALIDATING',updated_at=? WHERE delegation_id=?",
                (iso(now), claim["delegation_id"]),
            )
            c.execute(
                "UPDATE lanes SET state='RECONCILE',current_delegation_id=NULL,updated_at=? WHERE lane=?",
                (iso(now), claim["lane"]),
            )
            c.execute(
                "UPDATE dispatch_outbox SET state='VALIDATING',updated_at=? WHERE lease_id=?",
                (iso(now), lease_id),
            )
            self._event(
                c, claim["lane"], "VALIDATING", now,
                delegation_id=claim["delegation_id"], objective_id=claim["objective_id"],
                lease_id=lease_id, dispatch_id=claim["dispatch_id"],
                idempotency_key=claim["idempotency_key"], fencing_token=claim["fencing_token"],
                control_head=claim["control_head"], payload=validation_payload,
            )
    ...
    def evaluator_verdict(
        ...
        with self.tx() as c:
            prior = c.execute(
                "SELECT lease_id,payload_json FROM events WHERE event_type='EVALUATOR_VERDICT' ORDER BY seq"
            ).fetchall()
            for event in prior:
                payload = json.loads(event["payload_json"] or "{}")
            ...
            c.execute(
                "UPDATE claims SET status=?,terminal_reason=? WHERE lease_id=?",
                (terminal_state, json.dumps(verdict_payload, sort_keys=True), lease_id),
            )
            c.execute(
                "UPDATE delegations SET state=?,updated_at=? WHERE delegation_id=?",
                (terminal_state, iso(now), claim["delegation_id"]),
            )
            c.execute(
                "UPDATE dispatch_outbox SET state=?,updated_at=? WHERE lease_id=?",
                (terminal_state, iso(now), lease_id),
            )
            self._event(
                c, claim["lane"], "EVALUATOR_VERDICT", now,
                delegation_id=claim["delegation_id"], objective_id=claim["objective_id"],
                lease_id=lease_id, dispatch_id=claim["dispatch_id"],
                idempotency_key=claim["idempotency_key"], fencing_token=claim["fencing_token"],
                control_head=claim["control_head"], payload=verdict_payload,
            )
            self._event(
                c, claim["lane"], terminal_state, now,
                delegation_id=claim["delegation_id"], objective_id=claim["objective_id"],
                lease_id=lease_id, dispatch_id=claim["dispatch_id"],
                idempotency_key=claim["idempotency_key"], fencing_token=claim["fencing_token"],
                control_head=claim["control_head"],
                payload={"verdict_id": verdict_id, "candidate_digest": candidate_digest, "evaluator_verdict": verdict},
            )
            return terminal_state

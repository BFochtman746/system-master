...
export function normalizeA01Dispatch({ workflow = A01_GATEWAY_WORKFLOW, ref, inputs }) {
  ...
  // The workflow currently requires continuation fields. These values are not invented here: they are the
  // existing repository-defined A-01 route fallbacks, made explicit in the normalized dispatch packet.
...
export class GitHubWorkflowDispatchRestTransport {
  constructor({ owner, repo, tokenProvider = null, fetchImpl = globalThis.fetch, apiBase = 'https://api.github.com' }) {
    ...
    this.owner = owner; this.repo = repo; this.tokenProvider = tokenProvider; this.fetchImpl = fetchImpl; this.apiBase = apiBase.replace(/\/$/, '');
  }
  ...
  async request(method, path, body = undefined) {
    ...
    let response;
    try { response = await this.fetchImpl(`${this.apiBase}${path}`, { method, headers, body: body === undefined ? undefined : JSON.stringify(body) }); }
    catch (cause) { fail('DISPATCH_NETWORK_AMBIGUOUS', 'GitHub request failed before a definitive response', { cause }); }
    const text = await response.text();
    let parsed = null; if (text) { try { parsed = JSON.parse(text); } catch { parsed = text; } }
    if (!response.ok) fail('DISPATCH_GITHUB_API_ERROR', typeof parsed === 'object' && parsed?.message ? parsed.message : `GitHub API ${response.status}`, { status: response.status, body: parsed });
  ...
  async dispatch(workflow, ref, inputs) {
    return this.request('POST', `/repos/${encodeURIComponent(this.owner)}/${encodeURIComponent(this.repo)}/actions/workflows/${encodeURIComponent(workflow)}/dispatches`, { ref, inputs });
  }
...
export async function dispatchA01AndCorrelate({ transport, ref = 'main', inputs, pollAttempts = 12, pollIntervalMs = 1000 }) {
  if (!transport || typeof transport.snapshotRuns !== 'function' || typeof transport.dispatch !== 'function' || typeof transport.jobs !== 'function') throw new TypeError('workflow dispatch transport required');
  ...
  if (response?.status !== 204) fail('DISPATCH_HTTP_RESULT_INVALID', `workflow_dispatch expected HTTP 204, received ${response?.status ?? '<missing>'}`);

  for (let attempt = 0; attempt < pollAttempts; attempt += 1) {
    const after = await transport.snapshotRuns(packet.workflow, packet.ref);
    const candidates = after.filter((run) => !priorIds.has(run.id) && run.event === 'workflow_dispatch' && run.head_branch === packet.ref && new Date(run.created_at).getTime() >= new Date(requestedAt).getTime() - 2000);
    if (candidates.length > 1) fail('DISPATCH_CORRELATION_AMBIGUOUS', 'multiple new workflow runs match dispatch boundary', { run_ids: candidates.map((run) => run.id) });
    if (candidates.length === 1) {
      ...
      return Object.freeze({
        ...
        workflow_run: Object.freeze({ id: run.id, url: run.html_url ?? null, status: run.status ?? null, conclusion: run.conclusion ?? null, head_sha: run.head_sha ?? null, created_at: run.created_at ?? null }),
        jobs: Object.freeze(jobs.map((job) => Object.freeze({ id: job.id, name: job.name, status: job.status, conclusion: job.conclusion ?? null })))
      });
    }

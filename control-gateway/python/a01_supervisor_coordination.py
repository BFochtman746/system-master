...
def validate_coordination_contract(handoff: dict[str, Any], contract: dict[str, Any]) -> None:
    ...
    if not isinstance(contract["graph_version"], int) or contract["graph_version"] < 1:
    ...
    if not isinstance(contract["max_concurrency"], int) or not 1 <= contract["max_concurrency"] <= 64:
...
class SupervisorCoordinationAdapter:
    """Durable dependency, concurrency, and cancellation gate for A-01 supervisor work.

    The CG-008 handoff remains immutable. This layer binds extra scheduling semantics
    to its handoff_digest and persists them in the same SQLite coordination database.
    Claim eligibility and capacity are evaluated inside the same BEGIN IMMEDIATE
    transaction that creates the supervisor lease and dispatch outbox intent.
    """
    ...
    def claim(self, handoff: dict[str, Any], contract: dict[str, Any], now: Optional[dt.datetime] = None, lease_seconds: int = 300):
        ...
        with self.store.tx() as c:
            ...
            live_for_resource = c.execute(
                "SELECT COUNT(*) n FROM claims cl JOIN coordination_tasks ct ON ct.delegation_id=cl.delegation_id WHERE cl.released_at IS NULL AND ct.resource_key=?",
                (task["resource_key"],),
            ).fetchone()["n"]
    ...
    def request_cancel(self, delegation_id: str, reason: str, now: Optional[dt.datetime] = None, cascade: Optional[bool] = None) -> dict[str, Any]:
        ...
        with self.store.tx() as c:
            ...
            use_cascade = root["cancellation_policy"] == "CASCADE_DEPENDENTS" if cascade is None else bool(cascade)
            targets = self._descendants_locked(c, delegation_id) if use_cascade else []
    ...
    def audit_coordination_invariants(self) -> list[str]:
        problems: list[str] = []
        cycle = self._find_cycle_locked(self.store.conn)
        if cycle:
            problems.append("DEPENDENCY_CYCLE:" + "->".join(cycle))
        for row in self.store.conn.execute("SELECT resource_key,max_concurrency FROM coordination_resource_limits").fetchall():
            live = self.store.conn.execute(
                "SELECT COUNT(*) n FROM claims cl JOIN coordination_tasks ct ON ct.delegation_id=cl.delegation_id WHERE cl.released_at IS NULL AND ct.resource_key=?",
                (row["resource_key"],),
            ).fetchone()["n"]
            if int(live) > int(row["max_concurrency"]):
                problems.append(f"RESOURCE_LIMIT_EXCEEDED:{row['resource_key']}")
        cancelled_live = self.store.conn.execute(
    ...
    def _descendants_locked(self, c, root: str) -> list[str]:
        result: list[str] = []
        seen = {root}
        queue = [root]
        while queue:
            ...
            children = [r["delegation_id"] for r in c.execute(
                "SELECT delegation_id FROM coordination_dependencies WHERE dependency_id=? ORDER BY delegation_id",
                (current,),
            ).fetchall()]

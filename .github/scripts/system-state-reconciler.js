...
if (selftest) {
  ...
  const ids = new Set((topology.canonical_internal_systems || []).map((s) => s.system_id));
  const retired = new Set((topology.retired_systems || []).map((s) => s.system_id));
  const checks = [
    ['product_root', authority.product_root === 'SYSTEM_MASTER'],
    ['documents_is_active_peer', ids.has('DOCUMENTS')],
    ['prose_not_active', !ids.has('PROSE')],
    ['prose_retired', retired.has('PROSE')],
...
for (const required of ['CORE', 'LEARNING', 'BOOK', 'DOCUMENTS']) if (!activeIds.has(required)) finding('ERROR', 'CANONICAL_SYSTEM_MISSING', `canonical active system missing: ${required}`);
if (activeIds.has('PROSE')) finding('ERROR', 'RETIRED_SYSTEM_ACTIVE', 'PROSE must not remain in canonical_internal_systems after retirement');
const retiredSystems = topology.retired_systems || [];
const retiredById = Object.fromEntries(retiredSystems.map((s) => [s.system_id, s]));
if (!retiredById.PROSE) finding('ERROR', 'RETIREMENT_RECORD_MISSING', 'PROSE retirement is absent from topology');

const validOwnerPaths = new Set(['SYSTEM_MASTER', 'SYSTEM_MASTER/SHARED_INFRASTRUCTURE', 'SYSTEM_MASTER/SHARED_INFRASTRUCTURE/A01']);
for (const s of systems) validOwnerPaths.add(`SYSTEM_MASTER/${s.system_id}`);
for (const o of obligations) {
  ...
  if (o.owner_path === 'SYSTEM_MASTER/BOOK/PROSE' || o.owner_path.startsWith('SYSTEM_MASTER/BOOK/PROSE/')) finding('ERROR', 'RETIRED_OWNER_CURRENT_WORK', `current obligation still routes to retired Prose owner: ${o.obligation_id}`);
  const directValid = validOwnerPaths.has(o.owner_path) || o.owner_path.startsWith('SYSTEM_MASTER/SHARED_INFRASTRUCTURE/');
...
if (ownerFiles.PROSE) finding('ERROR', 'RETIRED_OWNER_SCHEDULED', 'Second Shift owner_files must not contain PROSE');
...
for (const [lane, rel] of Object.entries(ownerFiles)) {
  ...
  if (!system) finding('ERROR', 'SECOND_SHIFT_UNKNOWN_ACTIVE_SYSTEM', `Second Shift lane ${lane} is not an active canonical system`);
  const expectedPath = `SYSTEM_MASTER/${lane}`;
  if (data.owner_path !== expectedPath) finding('ERROR', 'SECOND_SHIFT_OWNER_PATH_MISMATCH', `${lane} owner_path mismatch`, { owner_path: data.owner_path, expected: expectedPath });
  ...
  owners[lane] = {
    ...
    open_obligations: obligations.filter((o) => o.owner_path === data.owner_path && !terminal.has(o.state)).map((o) => ({ obligation_id: o.obligation_id, state: o.state, objective: o.objective })),
...
for (const [lane, rel] of Object.entries(repairRegistry.owner_files || {})) {
  ...
  if (lane === 'PROSE' && (inbox.active_transactions || []).length) finding('ERROR', 'RETIRED_PROSE_REPAIR_ACTIVE', 'retired Prose repair inbox still contains active transactions');
...
const derived = {
  ...
  active_systems: systems.map((s) => s.system_id),
  retired_systems: retiredSystems.map((s) => ({ system_id: s.system_id, successor_system_id: s.successor_system_id || null, retirement_record: s.retirement_record || null })),
  owners,
...
console.log(`active_systems=${derived.active_systems.join(',')}`);

...
function normalizedDisplayKey(row, displayKey) {
  ...
  if (value === 'RESERVED' && row.module_key === row.requirement_or_capability_id) return row.module_key;
  return value;
}
...
function main() {
  let matrix;
  try {
    matrix = JSON.parse(execFileSync(process.execPath, [GENERATOR, '--json'], { cwd: ROOT, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }));
  } catch (error) {
  ...
  if (committed.rows.size !== matrix.rows.length) fail(`row-count drift: committed=${committed.rows.size} live=${matrix.rows.length}`);
  for (const row of matrix.rows) {
    ...
    if (displayKey !== row.module_key) fail(`capability/requirement key drift for ${id}: committed=${observed.key} live=${row.module_key}`);
    const expectedOwner = row.canonical_owner || UNPOPULATED;
    if (observed.owner !== expectedOwner) fail(`owner drift for ${id}: committed=${observed.owner} live=${expectedOwner}`);
    if (observed.state !== row.current_state) fail(`state drift for ${id}: committed=${observed.state} live=${row.current_state}`);
  }
  for (const id of committed.rows.keys()) if (!expectedIds.has(id)) fail(`committed matrix contains non-current inventory row: ${id}`);
  const committedCounts = {};
  for (const row of committed.rows.values()) committedCounts[row.state] = (committedCounts[row.state] || 0) + 1;
  ...
  if (committed.next !== matrix.exact_next_active_gap) fail(`next-gap drift: committed=${committed.next || 'NONE'} live=${matrix.exact_next_active_gap || 'NONE'}`);
  for (const requiredId of [matrix.authority_id, matrix.topology_id, matrix.allocation_id, matrix.crosswalk_id]) {
    if (!markdown.includes(`\`${requiredId}\``)) fail(`committed matrix missing current authority identifier: ${requiredId}`);
  }

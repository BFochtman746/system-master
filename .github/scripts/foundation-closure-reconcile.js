...
function evidenceState(entry, row, authority) {
  if (!entry) {
    return { state: 'NO_REGISTERED_EVIDENCE', current: false, drift: [], evidence_refs: [] };
  }
...
function effectiveState(row, evidence) {
  ...
  return row.current_state === COMPLETE && evidence.current ? COMPLETE : GAP;
}
...
function build() {
  ...
  const censusAuthorityRef = census.current_authority_ref || census.architecture_authority;
  const censusTopologyRef = census.current_topology_ref || census.topology_ref;
  const censusOwnerOrder = census.initial_owner_order || census.active_owner_order;
  const censusRetired = census.retired_systems || census.retired_owner_exclusions || [];
  ...
  if (censusAuthorityRef !== AUTHORITY_PATH) fail('CENSUS_AUTHORITY_POINTER_DRIFT', censusAuthorityRef || 'null');
  if (censusTopologyRef !== topologyPath) fail('CENSUS_TOPOLOGY_POINTER_DRIFT', `${censusTopologyRef || 'null'}!=${topologyPath}`);
  if (census.current_authority_id && census.current_authority_id !== authority.authority_id) {
    fail('CENSUS_AUTHORITY_ID_DRIFT', `${census.current_authority_id}!=${authority.authority_id}`);
  }
  ...
  if (census.current_topology_id && census.current_topology_id !== topology.topology_id) {
    fail('CENSUS_TOPOLOGY_ID_DRIFT', `${census.current_topology_id}!=${topology.topology_id}`);
  }
  ...
  if (!sameArray(censusOwnerOrder, activeOwners)) {
    fail('CENSUS_ACTIVE_OWNER_SET_DRIFT', `${JSON.stringify(censusOwnerOrder)}!=${JSON.stringify(activeOwners)}`);
  }
  for (const id of retired) {
    if (activeOwners.includes(id)) fail('RETIRED_OWNER_ACTIVE', id);
    if (!censusRetired.includes(id)) fail('RETIRED_OWNER_NOT_EXCLUDED', id);
  }
  ...
  return {
    ...
    census_rebase_id: census.rebase_id || null,

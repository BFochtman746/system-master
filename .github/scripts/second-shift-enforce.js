...
function validateLedger(ledger, lane, shiftDate, owner, schema) {
  const findings = [];
  const add = (severity, type, message, detail = {}) => findings.push({ severity, type, lane, message, ...detail });
  ...
  for (const e of ledger.events) {
    ...
    if (e.event_type === 'CLAIMED') {
      ...
      if (lease) add('ERROR', 'OVERLAPPING_MUTATION_CLAIMS', 'CLAIMED event overlaps an unresolved lease', { event_id: e.event_id, prior_lease_id: lease.lease_id });
      lease = { lease_id: e.lease_id, idempotency_key: e.idempotency_key, delegation_id: e.delegation_id, control_head: e.control_head };
    } else if (e.event_type === 'RUNNING' || e.event_type === 'PROGRESS') {
      if (!lease || lease.delegation_id !== e.delegation_id || lease.control_head !== e.control_head) add('ERROR', `${e.event_type}_WITHOUT_CLAIM`, `${e.event_type} lacks a matching live claim`, { event_id: e.event_id });
      ...
      if (!lease || lease.lease_id !== e.lease_id || lease.idempotency_key !== e.idempotency_key) add('ERROR', 'HEARTBEAT_WITHOUT_CLAIM', 'HEARTBEAT does not match live lease', { event_id: e.event_id });
    } else if (['COMPLETED', 'BLOCKED', 'STALE'].includes(e.event_type)) {
      if (lease && lease.delegation_id === e.delegation_id) lease = null;
...
for (const [lane, rel] of laneEntries) {
  ...
  if (watchdog && inShift && (ny.minute >= 28 || ny.hour > 0)) {
    ...
    else findings.push(...validateLedger(readJson(eventRel), lane, ny.date, owner, schema));
  }
}

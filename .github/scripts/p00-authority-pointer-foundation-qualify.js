...
function main() {
  ...
  assert(typeof obligation.registry_id === 'string' || typeof obligation.obligation_registry_id === 'string' || Array.isArray(obligation.obligations), 'P00_OBLIGATION_REGISTRY_SHAPE');

  const contract = fs.readFileSync(abs(PATHS.contract), 'utf8');
  for (const required of [
    EXPECTED.authorityId,
    EXPECTED.topologyId,
    EXPECTED.crosswalkId,
    'FOUNDATION-CLOSURE-EVIDENCE-REGISTRY-001',
    'P04 content-addressed authority bootstrap'
  ...
  if (process.env.P00_SUBJECT_SHA) assert(process.env.P00_SUBJECT_SHA === subjectSha, 'P00_SUBJECT_SHA_ENV_MISMATCH', `${process.env.P00_SUBJECT_SHA}:${subjectSha}`);
  ...
  const result = {
    qualification_id: 'P00-AUTHORITY-POINTER-FOUNDATION-QUALIFICATION-002',

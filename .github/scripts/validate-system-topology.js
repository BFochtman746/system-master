...
if (byId.PROSE.parent_id !== 'BOOK') fail('PROSE must be a child of BOOK');
if (ownerPath(byId.PROSE) !== 'SYSTEM_MASTER/BOOK/PROSE') fail('PROSE owner_path must be SYSTEM_MASTER/BOOK/PROSE');
if (byId.PROSE.classification !== 'ACTIVE_CHILD_SPECIALIST_SUBSYSTEM') fail('PROSE must be classified ACTIVE_CHILD_SPECIALIST_SUBSYSTEM');
if (!byId.PROSE.control_ref) fail('PROSE must declare its live specialist control_ref');
...
if (!childIds.has('PROSE')) fail('child_system_ids must contain PROSE');
...
if (history.prose_retirement_current_effect !== 'SUPERSEDED_BY_ADR_0004') fail('historical Prose retirement current effect must be superseded by ADR-0004');
...
if (laneMap['LITERARY-PROSE'] !== 'BOOK') fail('LITERARY-PROSE execution lane must map to BOOK');
if (laneMap['BOOK-EVAL-LEMONADE-001'] !== 'BOOK') fail('BOOK-EVAL-LEMONADE-001 execution lane must map to BOOK');
...
for (const s of systems) activeOwnerPaths.add(ownerPath(s));
...
if ((secondShift.coverage_routes || {})['SYSTEM_MASTER/BOOK/PROSE'] !== 'BOOK') fail('Second Shift must route SYSTEM_MASTER/BOOK/PROSE through BOOK');
...
console.log('child_systems=BOOK>PROSE');
console.log('prose_execution_lane=BOOK');
console.log('historical_prose_retirement=current_effect_superseded');

...
const files = {
  ...
  base_fixtures: 'qualification/book-system/version-and-rollback-001/BOOK-SYSTEM-VERSION-AND-ROLLBACK-001-FIXTURES.json',
  authority_fixtures: 'qualification/book-system/version-and-rollback-001/BOOK-SYSTEM-VERSION-AND-ROLLBACK-001-FIXTURES-v2.json',
...
const baseFixtures = JSON.parse(fs.readFileSync(path.join(workspace, files.base_fixtures), 'utf8'));
const authorityFixtures = JSON.parse(fs.readFileSync(path.join(workspace, files.authority_fixtures), 'utf8'));
...
function setup(customState = null, options = {}) {
  const initial = rt.createVersionLedger(customState || baseFixtures.parent_state_template, options);
...
function manuscriptV(state, version, hexChar) {
  ...
  next.manuscripts.push({
    ...
    parent_version_ref: next.active.canonical_manuscript_ref, change_set_ref: `CHANGE-${version}`, created_by: 'BOOK_SYSTEM_PARENT', created_at: '2026-09-09T00:02:00Z',
  });
...
function targetSnapshot(ledger, state) { return Object.values(ledger.state_snapshots).find(x => x.state_version === state.state_version && x.state_digest === state.state_digest); }
...
function authorizedPost(parent, mutator) {
  const post = clone(parent); delete post.state_digest; post.state_version = parent.state_version + 1; mutator(post); return rt.sealState(post);
}
function authorityReceipt(parent, post, kind, overrides = {}) {
  const idField = kind === 'LIFECYCLE_TRANSITION' ? 'transition_receipt_id' : kind === 'AUTHOR_DECISION' ? 'decision_receipt_id' : kind === 'EXPORT_RELEASE' ? 'release_receipt_id' : 'receipt_id';
  return { [idField]: `AUTH-${kind}-001`, pre_parent_state_version: parent.state_version, pre_parent_state_digest: parent.state_digest, post_parent_state_version: post.state_version, post_parent_state_digest: post.state_digest, ...overrides };
}
function authorizedRequest(parent, ledger, post, kind = 'LIFECYCLE_TRANSITION', overrides = {}) {
  return {
    request_id: 'VR-AUTH-001', idempotency_key: 'VR-AUTH-IDEM-001', actor_class: 'PARENT_SYSTEM',
    expected_state_version: parent.state_version, expected_state_digest: parent.state_digest,
    expected_ledger_version: ledger.ledger_version, expected_ledger_digest: rt.digestLedger(ledger),
    authority_kind: kind, authority_receipt: authorityReceipt(parent, post, kind), committed_parent_state: post, active_version_bindings: bindingMap(post),
    ...overrides,
  };
}
function recordLifecycleStatus(parent, ledger, status) {
  const post = authorizedPost(parent, p => { p.book_project.status = status; });
  const request = authorizedRequest(parent, ledger, post, 'LIFECYCLE_TRANSITION');
  return { post, request, out: rt.recordAuthorizedParentSuccessor({ parentState: parent, versionLedger: ledger, request }) };
}
...
test('VR014_FROZEN_CONTENT_MUTATION_REJECTED', () => { const base=clone(baseFixtures.parent_state_template); base.book_project.status='EXPORT_FROZEN'; const s=setup(base); const p=manuscriptV(s.parent,'V2','b'); const r=successorRequest(s.parent,s.ledger,p); expectError(()=>rt.commitSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'FROZEN_OR_PUBLISHED_CONTENT_MUTATION_FORBIDDEN'); });
test('VR015_STATUS_CHANGE_WITHOUT_LIFECYCLE_AUTHORITY_REJECTED', () => { const s=setup(); const p=clone(s.parent); delete p.state_digest; p.book_project.status='STRUCTURAL_REVIEW'; const r=successorRequest(s.parent,s.ledger,p); expectError(()=>rt.commitSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'LIFECYCLE_STATUS_CHANGE_REQUIRES_QUALIFIED_ENGINE_FIRST'); });
test('VR016_DIRECT_STATUS_CHANGE_REJECTED_EVEN_WITH_RECEIPT', () => { const s=setup(); const p=clone(s.parent); delete p.state_digest; p.book_project.status='STRUCTURAL_REVIEW'; const life={transition_receipt_id:'LIFE-001',from_status:'DRAFTING',to_status:'STRUCTURAL_REVIEW',pre_parent_state_version:s.parent.state_version,pre_parent_state_digest:s.parent.state_digest}; const r=successorRequest(s.parent,s.ledger,p,{lifecycle_transition_receipt:life}); expectError(()=>rt.commitSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'LIFECYCLE_STATUS_CHANGE_REQUIRES_QUALIFIED_ENGINE_FIRST'); });
...
test('VR029_FROZEN_RESTORE_REQUIRES_REOPEN', () => { const base=clone(baseFixtures.parent_state_template); base.book_project.status='EXPORT_FROZEN'; const s=setup(base); const snap=s.ledger.state_snapshots[s.ledger.current_snapshot_id]; const r=restoreRequest(s.parent,s.ledger,snap); expectError(()=>rt.restoreAsNewSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'FROZEN_OR_PUBLISHED_REOPEN_REQUIRED'); });
test('VR030_FULL_STATE_STATUS_CHANGE_REQUIRES_LIFECYCLE_FIRST', () => { const s=setup(); const lifecycle=recordLifecycleStatus(s.parent,s.ledger,'STRUCTURAL_REVIEW'); const target=targetSnapshot(lifecycle.out.version_ledger,s.parent); const rr=restoreRequest(lifecycle.out.parent_state,lifecycle.out.version_ledger,target,{mode:'RESTORE_FULL_STATE'}); expectError(()=>rt.restoreAsNewSuccessor({parentState:lifecycle.out.parent_state,versionLedger:lifecycle.out.version_ledger,request:rr}),'FULL_STATE_RESTORE_REQUIRES_PRIOR_LIFECYCLE_TRANSITION'); });
...
test('VR038_MULTIPLE_NEW_VERSIONS_SAME_FAMILY_REJECTED', () => { const s=setup(); const p=clone(s.parent); delete p.state_digest; p.manuscripts.push({manuscript_id:'MANUSCRIPT-001',version_id:'V2',artifact_digest:'b'.repeat(64),authority_state:'CANONICAL',parent_version_ref:'MANUSCRIPT-001:V1',change_set_ref:'CHANGE-V2',created_by:'BOOK_SYSTEM_PARENT',created_at:'2026-09-09T00:02:00Z'}); p.manuscripts.push({manuscript_id:'MANUSCRIPT-001',version_id:'V3',artifact_digest:'c'.repeat(64),authority_state:'CANONICAL',parent_version_ref:'MANUSCRIPT-001:V2',change_set_ref:'CHANGE-V3',created_by:'BOOK_SYSTEM_PARENT',created_at:'2026-09-09T00:03:00Z'}); p.active.canonical_manuscript_ref='MANUSCRIPT-001:V3'; const r=successorRequest(s.parent,s.ledger,p); expectError(()=>rt.commitSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'MULTIPLE_NEW_VERSIONS_SAME_FAMILY_ONE_COMMIT'); });
test('VR039_INITIAL_MULTI_VERSION_WITHOUT_LINEAGE_REJECTED', () => { const p=clone(baseFixtures.parent_state_template); p.manuscripts.push({manuscript_id:'MANUSCRIPT-001',version_id:'V2',artifact_digest:'b'.repeat(64),authority_state:'CANONICAL',parent_version_ref:'V1',change_set_ref:'CHANGE-V2',created_by:'BOOK_SYSTEM_PARENT',created_at:'2026-09-09T00:02:00Z'}); p.active.canonical_manuscript_ref='MANUSCRIPT-001:V2'; expectError(()=>rt.createVersionLedger(p),'INITIAL_LINEAGE_REQUIRED'); });
test('VR040_ACTIVE_POINTER_AMBIGUITY_REJECTED', () => { const p=clone(baseFixtures.parent_state_template); p.governing_briefs.push({brief_id:'BRIEF-001',version:2,author_intent:'Second',form:'NOVEL',genre:'SPECULATIVE',audience:'ADULT',voice_goals:['DISTINCT'],hard_constraints:['NO_SILENT_CANON_CHANGE']}); expectError(()=>rt.createVersionLedger(p),'ACTIVE_POINTER_VERSION_AMBIGUOUS'); });
test('VR041_AUTHORIZED_LIFECYCLE_SUCCESSOR_RECORDED_WITHOUT_REEXECUTION', () => { const s=setup(); const x=recordLifecycleStatus(s.parent,s.ledger,'STRUCTURAL_REVIEW'); assert(x.out.disposition==='COMMITTED'&&x.out.parent_state.book_project.status==='STRUCTURAL_REVIEW','AUTHORIZED_LIFECYCLE_INGEST_FAIL'); assert(x.out.receipt.authority_kind==='LIFECYCLE_TRANSITION','AUTHORITY_KIND_NOT_RECORDED'); });
test('VR042_AUTHORITY_RECEIPT_PRE_IDENTITY_MISMATCH_REJECTED', () => { const s=setup(); const post=authorizedPost(s.parent,p=>{p.book_project.status='STRUCTURAL_REVIEW';}); const r=authorizedRequest(s.parent,s.ledger,post,'LIFECYCLE_TRANSITION'); r.authority_receipt.pre_parent_state_digest='0'.repeat(64); expectError(()=>rt.recordAuthorizedParentSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'PARENT_AUTHORITY_RECEIPT_PRE_IDENTITY_MISMATCH'); });
test('VR043_AUTHORITY_RECEIPT_POST_IDENTITY_MISMATCH_REJECTED', () => { const s=setup(); const post=authorizedPost(s.parent,p=>{p.book_project.status='STRUCTURAL_REVIEW';}); const r=authorizedRequest(s.parent,s.ledger,post,'LIFECYCLE_TRANSITION'); r.authority_receipt.post_parent_state_digest='0'.repeat(64); expectError(()=>rt.recordAuthorizedParentSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'PARENT_AUTHORITY_RECEIPT_POST_IDENTITY_MISMATCH'); });
test('VR044_UNKNOWN_AUTHORITY_KIND_REJECTED', () => { const s=setup(); const post=authorizedPost(s.parent,p=>{p.book_project.status='STRUCTURAL_REVIEW';}); const r=authorizedRequest(s.parent,s.ledger,post,'LIFECYCLE_TRANSITION',{authority_kind:'UNKNOWN_AUTHORITY'}); expectError(()=>rt.recordAuthorizedParentSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'UNKNOWN_PARENT_AUTHORITY_KIND'); });
test('VR045_AUTHORIZED_SUCCESSOR_REPLAY_NO_SECOND_SNAPSHOT', () => { const s=setup(); const post=authorizedPost(s.parent,p=>{p.book_project.status='STRUCTURAL_REVIEW';}); const r=authorizedRequest(s.parent,s.ledger,post,'LIFECYCLE_TRANSITION'); const first=rt.recordAuthorizedParentSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}); const count=Object.keys(first.version_ledger.state_snapshots).length; const replay=rt.recordAuthorizedParentSuccessor({parentState:first.parent_state,versionLedger:first.version_ledger,request:r}); assert(replay.disposition==='REPLAY'&&Object.keys(replay.version_ledger.state_snapshots).length===count,'AUTHORIZED_REPLAY_FAIL'); });
test('VR046_AUTHORIZED_SUCCESSOR_APPEND_ONLY_MUTATION_REJECTED', () => { const s=setup(); const post=authorizedPost(s.parent,p=>{p.research_evidence_links[0].rights_class='MUTATED';}); const r=authorizedRequest(s.parent,s.ledger,post,'CANONICAL_STATE_MUTATION'); expectError(()=>rt.recordAuthorizedParentSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}),'APPEND_ONLY_HISTORY_MUTATION_FORBIDDEN'); });
test('VR047_INTEGRATION_RUNTIME_SUCCESSOR_RECORDED_WITH_ACTIVE_BINDINGS_UNCHANGED', () => { const s=setup(); const before=clone(s.ledger.state_snapshots[s.ledger.current_snapshot_id].active_version_bindings); const post=authorizedPost(s.parent,p=>{p.integration_proposals.push({proposal_id:'PROPOSAL-002',source_project_or_lane:'SYSTEM_MASTER/BOOK/PROSE',source_subject_sha:'2222222222222222222222222222222222222222',capability_id:'PROSE_ANALYSIS_AND_REVISION.ANALYZE_PASSAGE_OR_UNIT',admission_state:'PROPOSED'});}); const r=authorizedRequest(s.parent,s.ledger,post,'INTEGRATION_RUNTIME'); const out=rt.recordAuthorizedParentSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}); const after=out.version_ledger.state_snapshots[out.version_ledger.current_snapshot_id].active_version_bindings; assert(stable(before)===stable(after),'INTEGRATION_ACTIVE_BINDINGS_CHANGED'); });
test('VR048_AUTHORIZED_SUCCESSOR_POST_STATE_BYTES_PRESERVED_EXACTLY', () => { const s=setup(); const post=authorizedPost(s.parent,p=>{p.book_project.status='STRUCTURAL_REVIEW';}); const r=authorizedRequest(s.parent,s.ledger,post,'LIFECYCLE_TRANSITION'); const out=rt.recordAuthorizedParentSuccessor({parentState:s.parent,versionLedger:s.ledger,request:r}); assert(stable(out.parent_state)===stable(post)&&out.parent_state.state_digest===post.state_digest,'AUTHORIZED_POST_BYTES_CHANGED'); });

const expected = authorityFixtures.cases.map(c => c.id);
...
for (const id of expected) { try { tests[id](); results.push({id,result:'PASS'}); } catch(e) { failed+=1; results.push({id,result:'FAIL',code:e&&e.code?e.code:'UNEXPECTED_ERROR',detail:String(e&&e.message?e.message:e)}); } }
const summary={
  qualification_id:'BOOK-SYSTEM-VERSION-AND-ROLLBACK-001', result_class:failed===0?'PASS':'FAIL', subject_sha:gitHead(), case_count:expected.length, pass_count:expected.length-failed, fail_count:failed,
  fixture_version:authorityFixtures.fixture_version, superseded_fixture_semantics:authorityFixtures.supersedes_case_semantics,
  contract_standing:contract.standing, research_standing:research.standing,
  authority:{ parent_system_commit_only:true, direct_lifecycle_status_change_allowed:false, authorized_successor_ingest_requires_exact_receipt:true, history_rewrite_allowed:false, author_approval_synthesis_allowed:false, publication_authority_synthesis_allowed:false, child_or_provider_canonical_write_allowed:false, document_engine_authority_created:false },
...
write('qualification-manifest.json',{qualification_id:summary.qualification_id,subject_sha:summary.subject_sha,evidence_files:['qualification-summary.json'],case_count:summary.case_count,result_class:summary.result_class,fixture_version:summary.fixture_version});

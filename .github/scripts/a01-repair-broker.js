...
function inboxRegistry() {
  return readJson(inboxRegistryPath);
}

function ownerPathForSystem(systemId) {
  ...
  if (systemId === 'PROSE') return 'SYSTEM_MASTER/BOOK/PROSE';
  return null;
}
...
function resolveProductOwner(workstreamId, topo = topology()) {
  ...
  const ownerPath = ownerPathForSystem(systemId);
...
function baseTransaction(input, classification, owner, state, route, createdAt) {
  ...
  return {
    ...
    target_inbox: targetInbox(owner.inbox_system_id),
...
function openTransaction(input, options = {}) {
  ...
  if (classification === 'PASS') {
    ...
    const transaction = baseTransaction(input, classification, owner, state, route, createdAt);
    ...
    return { broker_version: 1, action: 'OPEN', transaction, terminal: state === 'DEAD_LETTER', inbox_registry: registry.registry_id };
  ...
  return {
    ...
    action: 'OPEN',
    transaction,
    terminal: ['NO_ACTION', 'DEAD_LETTER'].includes(state),
    inbox_registry: registry.registry_id
  };
}
...
function finalizeTransaction(input, options = {}) {
  ...
  assert(tx.state === 'REPAIR_REQUEST_READY' || tx.state === 'CANDIDATE_PREQUAL_REQUIRED' || tx.state === 'CLAIMED', `transaction state not eligible for product repair finalization: ${tx.state || '<missing>'}`);
  ...
  const plan = planRepair({
    ...
    replacement_subject_sha: candidate.replacement_subject_sha,
    repair_branch: candidate.repair_branch || null,
    prequalification: candidate.prequalification || {},
    promotion_authorized: false,
    authoritative_pass: false
  });
  ...
  return {
    ...
    transaction: {
      ...tx,
      state: 'A01_REQUEUE_READY',
      route: 'REQUEUE_CHANGED_SHA',
      replacement_subject_sha: plan.replacement_subject_sha,
      repair_branch: candidate.repair_branch || null,
      prequalification_evidence_pointer: plan.prequalification?.evidence_pointer || null,
      replacement_request: plan.replacement_request,
      promotion_authorized: false,
      authoritative_pass: false,
      publication_authorized: false,
      production_authorized: false,
...
function cli(argv) {
  ...
  process.stdin.on('end', () => {
    try {
      ...
      let output;
      if (command === 'open') output = openTransaction(input);
      else if (command === 'finalize') output = finalizeTransaction(input);
      else throw new Error('usage: node a01-repair-broker.js <open|finalize> < input.json');
      process.stdout.write(JSON.stringify(output, null, 2) + '\n');
    } catch (error) {
      console.error(`A01_REPAIR_BROKER_ERROR: ${error.message}`);
      process.exit(1);
    }
  });
}
